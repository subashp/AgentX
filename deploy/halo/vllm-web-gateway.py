...
function submitPrompt(){if(send.disabled)return;if(form.requestSubmit)form.requestSubmit(send);else form.dispatchEvent(new Event('submit',{cancelable:true,bubbles:true}));}
prompt.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.isComposing){e.preventDefault();submitPrompt();}});
